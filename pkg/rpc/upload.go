...
func (s *Server) UploadLargeBif(stream pb.BifExamine_UploadLargeBifServer) error {
	req, tempBifPath, bifHash, err := s.saveBifStreamToFile(stream)
	if err != nil {
		return err
	}
	resp := &pb.UploadBifResponse{}
	defer os.Remove(tempBifPath)

	log.Info().
		Str("tempBifPath", tempBifPath).
		Str("fileHash", bifHash).
		Msg("Uploaded large bif")
...
func (s *Server) saveBifStreamToFile(stream pb.BifExamine_UploadLargeBifServer) (*pb.UploadBifRequest, string, string, error) {
	...
	// We're done writing all the chunks given to us
	file.Close()
